...
'''
class SubBracketPLayIn():
    """
    A region, or sub-bracket of 2 teams for a NCAA tournament
    ...
    winner : Team
        The    winner : Team
        The team from this region that is simulated to make the round of 32
    ...
    def __init__(self, region):
        ...
        # Set region
        self.region = region
        self.round1 = [] # first round will have 2 teams
        self.winner = None # Initalize as None until winner is set

    def initialize_playin_round(self, teams):
        ...
        self.team0A = teams[0]
        self.team0B = teams[1]
        self.Game1 = Game(self.team0A, self.team0B, 0)

    def run_play_in(self, sim_func):
        
        if not self.round1: raise Exception("Need to initialize first round before running")
        self.winner = sim_func(self.Game1)
        return self
...
class FinalFour():
    ...
    def set_matches(self, teams):
        ...
        self.round5=[self.Game1,self.Game2]
...
class Bracket():
    ...
    def __repr__(self): 
        ...
        return f"{header}{self.Finals}{self.W}{self.X}{self.Y}{self.Z}"
    ...
    def __init__(self, year):
        ...
        year : int
            Year of the NCAA tournament
        """
        valid_years = [y for y in range(1985, 2024)]
        valid_years.remove(2020)
        valid_years.remove(2021)

        if year not in valid_years:
            print('Picked season:',year)
            raise ValueError("Year must be between 1985 and 2023, the 2020 tournament was not played and my data is bad for 2021")
        ...
        for team in self.regions['W']: add_east(Team(name=team['Team'], seed=team['Seed']))
        for team in self.regions['X']: add_west(Team(name=team['Team'], seed=team['Seed']))
        for team in self.regions['Y']: add_midwest(Team(name=team['Team'], seed=team['Seed']))
        for team in self.regions['Z']: add_south(Team(name=team['Team'], seed=team['Seed']))


        self.W = SubBracket16('W').initialize_first_round(east_teams)
        self.X = SubBracket16('X').initialize_first_round(west_teams)
        self.Y = SubBracket16('Y').initialize_first_round(midwest_teams)
        self.Z = SubBracket16('Z').initialize_first_round(south_teams)
    ...
    def run_first_round(self, sim_func):
        ...
        self.W.run_first_round(sim_func)
        self.X.run_first_round(sim_func)
        self.Y.run_first_round(sim_func)
        self.Z.run_first_round(sim_func)
        
        self.round2 = \
            [team for game in self.W.round2 for team in [game.top_team, game.bottom_team]] + \
            [team for game in self.X.round2 for team in [game.top_team, game.bottom_team]] + \
            [team for game in self.Y.round2 for team in [game.top_team, game.bottom_team]] + \
            [team for game in self.Z.round2 for team in [game.top_team, game.bottom_team]]
    ...
    def run_second_round(self, sim_func):
        ...
        self.W.run_second_round(sim_func)
        self.X.run_second_round(sim_func)
        self.Y.run_second_round(sim_func)
        self.Z.run_second_round(sim_func)
        
        self.round3 = \
            [team for game in self.W.round3 for team in [game.top_team, game.bottom_team]] + \
            [team for game in self.X.round3 for team in [game.top_team, game.bottom_team]] + \
            [team for game in self.Y.round3 for team in [game.top_team, game.bottom_team]] + \
            [team for game in self.Z.round3 for team in [game.top_team, game.bottom_team]]
    ...
    def run_sweet_sixteen(self, sim_func):
        ...
        self.W.run_sweet_sixteen(sim_func)
        self.X.run_sweet_sixteen(sim_func)
        self.Y.run_sweet_sixteen(sim_func)
        self.Z.run_sweet_sixteen(sim_func)
        
        self.round4 = \
            [team for game in self.W.round4 for team in [game.top_team, game.bottom_team]] + \
            [team for game in self.X.round4 for team in [game.top_team, game.bottom_team]] + \
            [team for game in self.Y.round4 for team in [game.top_team, game.bottom_team]] + \
            [team for game in self.Z.round4 for team in [game.top_team, game.bottom_team]]
    ...
    def run_elite_eight(self, sim_func):
        ...
        # Run sweet sixteen round for all brackets
        self.W.run_elite_eight(sim_func)
        self.X.run_elite_eight(sim_func)
        self.Y.run_elite_eight(sim_func)
        self.Z.run_elite_eight(sim_func)
        
        # Place each team in final four
        region_names = ['W', 'X', 'Y', 'Z'] # Order of these two lists must match
        self.round5 = [self.W.winner, self.X.winner, self.Y.winner, self.Z.winner]
    ...
    def score_round(self, sim_func=None, verbose=True):
        """
        Calculates the number of games correct from the simulation and 
        that total score (32 points per round). Will run a new simulation
        as well if passed `sim_func` argument. 
        ...
        if len(self.round2)>0:
            ...
            n_correct.append(n_correct_round1)
            score=n_correct_round1
            scores.append(score)
            i=2

        if len(self.round3)>0:
            n_correct_round2 = len(set([team['Team'] for team in self.result['sweet16']]).intersection([team.name for team in self.round3]))
            n_correct.append(n_correct_round2)
            score=n_correct_round2*2
            scores.append(score)

            i=3

        if len(self.round4)>0:
            n_correct_round3 = len(set([team['Team'] for team in self.result['elite8']]).intersection([team.name for team in self.round4]))
            n_correct.append(n_correct_round3)
            score=n_correct_round3*4
            scores.append(score)

            i=4
      
        if len(self.round5)>0:
            n_correct_round4 = len(set([team['Team'] for team in self.result['final4']]).intersection([team.name for team in self.round5]))
            n_correct.append(n_correct_round4)
            score=n_correct_round4*8
            scores.append(score)

            i=5

        if len(self.round6)>0:
            n_correct_round5 = len(set([team['Team'] for team in self.result['championship']]).intersection([team.name for team in self.round6]))
            n_correct.append(n_correct_round5)
            score=n_correct_round5*16
            scores.append(score)
        ...
        if self.winner:
            ...
            correct_winner = int(self.winner.name == self.result['winner'])
            n_correct.append(correct_winner)
            score=correct_winner*32
            scores.append(score)
        ...
        if verbose:
            print(f"Number of games correct through round {i}: {total_correct}/63")
            print(f"Total Score: {self.total_score}/192")
